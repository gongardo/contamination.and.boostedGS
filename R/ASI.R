...
ASI <- function(alg_par, hyper_par){
  ...
  g <- hyper_par$g       # g, parameter of g prior.
  h <- hyper_par$h       # model prior parameter, prior over the model space??
  h_ini<-5/p
  phi_prop <- phi[1]
  phi_swap <- phi[2]
  ...
  else{ #Prior binomial beta over the model parameter, to be equal to the Scott and Berger; h[1]=1 and h[2]=1. In the simulation
    #they use an fixed h (paper)
    # binomial-beta
    h_alpha <- h[1]
    h_beta <- h[2]
  ...
  if (hyper_par$g_prior_type == "g") {
    ...
    bf <- bf_g_L #Calculo del BF
    log_llh <- log_llh_g_L ##Log marginal?? escribirla en funcion del BF tambien 
    
  }
  #if (hyper_par$g_prior_type == "g2") {
	  #This is the version of g-prior using BayesVarSel (recall, here g=n) 
  #  bf <- bf_g2_L #Calculo del BF
  #  log_llh <- log_llh_g2_L ##Log marginal?? escribirla en funcion del BF tambien 
    
  #}  
  if (hyper_par$g_prior_type == "gcont") {
	  #This is the version of g-prior with contamination and using BayesVarSel (recall, here g=n)
    bf <- bf_gcont2_L #Calculo del BF
    log_llh <- log_llh_gcont2_L ##Log marginal?? escribirla en funcion del BF tambien 
    
  }   
  if (hyper_par$g_prior_type == "ind") {
    
    bf <- bf_ind_L
    log_llh <- log_llh_ind_L
    
  }
  if (hyper_par$g_prior_type == "indcont") {
    
    bf <- bf_indcont_L
    log_llh <- log_llh_indcont_L
  ...
  for (t in 1:n_temp) {
    currs_temp <- matrix(runif(n = p*n_chain) < h_ini, nrow = n_chain, ncol = p)
