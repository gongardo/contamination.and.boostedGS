...
B.PMcurrent<- B.PM.cont(current.model[realindex], nofesp=sum(current.model)-sum(current.model[realindex]), eta=eta, p=p,X=X,y=y)
...
for (i in 1:N){
      for (j in 1:p){
        ...
        B.PMproposal<- B.PM.cont(proposal.model[realindex], nofesp=sum(proposal.model)-sum(proposal.model[realindex]), eta=eta, p=p,X=X,y=y)
        ratio<- B.PMproposal/(B.PMproposal+B.PMcurrent)
        
        if (runif(1)<ratio) {
          current.model[j]<- proposal.model[j]; B.PMcurrent<- B.PMproposal
        }
