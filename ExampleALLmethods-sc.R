#simulated contaminated experiment
#PARNI, ASI, TGS, wTGS and (three implementations of) Gibbs sampling
#here an example with n=500 so results are expected to be
#very close to the virtually contaminated experiment
#(file ExampleALLmethods-vc.R)

rm(list=ls())

mypath<- "~/Dropbox/parallelGibbs" #gonzalo
#mypath<- "~/Dropbox/Investigacion/parallelGibbs" #maria eugenia

setwd(paste(mypath,"/R", sep=""))

library(BayesVarSel)
library(doParallel)
#functions used in the parallel implementation of Gibbs
#corresponding to the contaminated experiment:
source("base-functions-ParallelGibbs.R") 
#function for the vectorized implementation of Gibbs :
source("bf_g_L_4GS.R") 
#functions to create the arguments to interact with PARNI, ASI, TGS and wTGS
#(strongly based on original PARNI "make_hyper_par.R"):
source("make_hyper_par.R")
#this contains the contaminated versions of gBF:
source("more_other_supportive_functions2.R")

#almost the original functions for PARNI and ASI:
source("ASI.R")
source("PARNI.R")
source("other_supportive_functions.R")

#functions for TGS and wTGS (almost the same as original but with fast implementation and contaminated implementation)
source("functions_for_BVS.R")


########
########
#How to use all these (simulated data)
########
########

#Simulated data:
set.seed(1234)
n<- 500; kT<- 4; p<- 1000
X<- matrix(rnorm(n*kT), nc=kT, nr=n)
y<- .1*X[,1]+.5*X[,2]+1*X[,3]+0*X[,4]+rnorm(n)
X<- cbind(X, matrix(rnorm(n*(p-kT)), nc=p-kT, nr=n))
#Note: the first 4 columns in X exactly coincide with
#those in X used in the file ExampleALLmethods-vc.R
#and the y's are also the same
#the rest of columns in X are independent on y
#now we 'hide' the true variables within all the others
#and realindex is a vector that states where they are in the new ordering
permute<- sample(1:p, p, replace=FALSE)
X<- X[, permute]
realindex<- rep(0, 4)
for (i in 1:4) realindex[i]<- which(permute==i)

########
########
#2. PARNI with simulated contaminated experiment
########
########

#We make hyper par, using Scott-Berger prior
hyper_par <- make_hyper_par(y = scale(y, scale=FALSE),  # response
                            X = scale(X),
							prior_type = 2 #2 is for original g-prior
							)


# PARNI
alg_par <- list(N = 6000,  # number of iterations
                Nb = 2000,  # burn-in period
                full_adap = FALSE, # whether to adapt full chain
                verbose = TRUE,
                kappa = 0.001,
                eps = 0.1/p,
                target_swap = 0.25, # Parallel tempering target swapping acceptance rate
                n_chain = 25,
                n_temp = 1,
                omega_adap = "kw",omega_par = c(-1, -0.5), # Kiefer-Wolfwitz
                # omega_adap = "rm",omega_par = c(-0.7, 0.65), # Robbins-Monro
                omega_init = 0.5,
                store_chains = FALSE, # whether to store the full chains
                # bal_fun = function(x) {x/(1+x)},
                bal_fun = function(x) { min(x,1) },
                # bal_fun = sqrt,
                use_rb = TRUE  # whether to use Raoo-Blackwellised PIPs
)

results <- PARNI(hyper_par = hyper_par, alg_par = alg_par)
results$rb_PIPs[realindex]
#[1] 0.0008660230 1.0000000000 1.0000000000 0.0001752907

########
########
#3. ASI with simulated contaminated experiment
########
########

alg_par <- list(N = 6000,
                Nb = 2000,
                full_adap = FALSE,
                verbose = TRUE,
                kappa = 0.001,
                eps = 0.1/hyper_par$p,
                phi = c(-0.7, -0.7),
                target_prop = 0.2,
                target_swap = 0.25,
                n_chain = 25,
                init_zeta = 0.5,
                n_temp = 1,
                store_chains = FALSE,
				use_rb = TRUE  # whether to use Raoo-Blackwellised PIPs
				)

results <- ASI(hyper_par = hyper_par, alg_par = alg_par)
results$rb_PIPs[realindex]
#[1] 0.0008769715 1.0000000000 1.0000000000 0.0001757255


########
########
#4. TGS with contaminated experiment (note prior_type=12)
########
########

hyper_par <- make_hyper_par(y = scale(y, scale=FALSE),  # response
                            X = scale(X),
							prior_type = 12)

output_TGS <- TGS(p=p, hyper_par=hyper_par, T=50000, burn_in=5000)
output_TGS$est_inclusion_probs[realindex]
#[1] 0.0010624510 1.0000000000 1.0000000000 0.0002712521


########
########
#5. wTGS with contaminated experiment (note prior_type=12)
########
########

output_wTGS <- wTGS(p=p, hyper_par=hyper_par, T=50000, burn_in=5000)
output_wTGS$est_inclusion_probs[realindex]
#[1] 0.0008597710 1.0000000000 1.0000000000 0.0001752113

########
########
#6. Gibbs sampling (component-by-component) with contaminated experiment
########
########

d.ini<- 5 #dimension of initial model (which is randomly chosen)
N<- 1000 #number of iterations
source("linear-Gibbs-logBF.R")
resumen.gibbs$inclprob[realindex]
#[1] 0.0008564389 1.0000000000 1.0000000000 0.0001756039

########
########
#7. Gibbs sampling (vectorized version) with contaminated experiment
########
########
source("Gibbs-vectorized.R")
resumen$inclprob[realindex]
#[1] 0.0008748789 1.0000000000 1.0000000000 0.0001784798


########
########
#8. Gibbs sampling (parallelized version of component-by-component) with contaminated experiment
########
########

N.ini<- 5
N<- 1000-50
esta.ini<- 100 
nofproc<- 5
distribute<- TRUE
#small explanation:
#in {1,2,...,p} (after the ordering estimated with N.ini iterations)
#the first {1,2,...,esta.ini} are sampled component-by-component
#the rest are distributed in parallel among nofproc cores
source("parallelGibbs-logBF-TwoProcessesv3.R")

resumen$inclprob.step2[realindex]
#[1] 0.0008531488 1.0000000000 1.0000000000 0.0001809475


